...
```{r}
toolik_meantemp_jitter <- ggplot(data = toolik_weather, aes(x = month, y = daily_air_temp_mean_c))+
  geom_jitter(aes(color = daily_air_temp_mean_c))+
  scale_color_gradientn(colors = c("cyan", "blue", "purple"))
toolik_meantemp_jitter

ggsave(here::here("figs", "toolik_meantemp_jitter.png"), height = 5, width = 6)
```
